    ...
    if (spec) {
      if (spec === 'between') {
        ...
        query[k] = {
          $in: cond.map(function(x) {
            ...
            else if (self.isDateProperty(model, prop, x)) return toDate(x);
            return x;
          }),
        };
      } else if (spec === 'nin') {
      ...
      if (cond === null) {
        ...
        if (self.isObjectIDProperty(model, prop, cond)) {
          cond = ObjectID(cond);
        } else if (self.isDateProperty(model, prop, cond)) {
          cond = toDate(cond);
        }
        query[k] = cond;
      }
...
MongoDB.prototype.isDateProperty = function(model, prop, value) {
  if (prop && ((prop.type === Date) || (Array.isArray(prop.type) && prop.type[0] === Date))) {
    return true;
  }  else {
    return false;
  }
